...
public class mapdisp {
	...
	public int[][] map = new int[dimensions][dimensions];
	...
	Image tileImage = new Image("images//chip//textures//BlankTile.png", 50, 50, true, true);
		...
		for (int x = 0; x < dimensions; x++) {
			for (int y = 0; y < dimensions; y++) {
				...
				if (map[x][y] == 1) {
					...
					chipsImage = new Image("images//chip//textures//chipItem.png", 50, 50, true, true);
	...
	public void drawLevel()
	{
		if (this.level1)
		{
			level level = new level(map, wallImage, scale, root, walls, gate, portal, blueKey, blueKeyWall, blueKeyImage, blueKeyWallImage, gateImage);
			level.simWorldLevelOne();
		}
		else 
		{
			level level = new level(map, wallImage, scale, root, walls, gate, portal, blueKey, blueKeyWall, blueKeyImage, blueKeyWallImage, gateImage);
			level.simWorldLevelTwo();
		}
	}
